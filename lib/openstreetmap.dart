...
/// OSM-compliant User-Agent: app name / package (per OSM tile policy).
const String _osmUserAgent =
    'com.ghostcorporation.mapsapp/1.0 (Flutter; +https://github.com)';
...
class _OpenStreetMapScreenState extends State<OpenStreetMapScreen> {
  ...
  Future<void> fetchCoordinatesPoint(String location) async {
    ...
    if (response.statusCode == 200) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text("Failed to fetch location.Try again later.")),
      );
    }
  }

  //Method to fetch route between the current location and the destination using the OSRM API.
  Future<void> _fetchRoute() async {
    if (_currentLocation != null && destination == null) return;
    final url = Uri.parse(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        ...
        child: Icon(Icons.my_location, size: 30, color: Colors.blue),
      ),
    );
  }
}
